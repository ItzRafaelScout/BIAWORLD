...
function toBgImg(name, color) {
    return color.split(" ").map(sprite => `url("img/bonzi/${sprite}.png")`).reverse().join(", ");
}
function updateAds() {
...
function bonzilog(id, name, html, color, text, single) {
    let icon = "";
    let $chatLogContent = $('#chat_log_content');
    let scrolled = $chatLogContent[0].scrollHeight - $chatLogContent[0].clientHeight - $chatLogContent.scrollTop() <= 1;
    let bgImg = toBgImg(name, color);
    icon = `<div class="log_icon" style="background-image: ${sanitize(bgImg)}"></div>`;
    let thisUser = `${id};${name};${color}`;
    
    if (thisUser !== lastUser || single) {
        $chatLogContent.append(`
            <hr>
        ...
        if(color.includes(" ")) {
            $chatLogContent.children().last().find(".log_icon")
                .css("background-size", "100px 80px, calc(var(--src-width) * var(--scale))");
        }
    ...
    if (scrolled) {
        $chatLogContent.scrollTop($chatLogContent[0].scrollHeight);
    }
}
function loadBonzis(a) {
    loadQueue.loadManifest([
        ...
        { id: "bonzi69", src: "./img/bonzi/deepfry.png" },
        { id: "bonziKamala", src: "./img/bonzi/kamala.png" },
        { id: "bonziShadow", src: "./img/bonzi/shadow.png" },
        { id: "FBI", src: "./img/bonzi/FBI.png" },
        { id: "biblicalaccurateangel", src: "./img/bonzi/biblicalaccurateangel.png" },
        { id: "eateot1", src: "./img/bonzi/eatot1.png" },
        { id: "maidtchi", src: "./img/bonzi/maidtchi.png" },
        { id: "mametchi", src: "./img/bonzi/mametchi.png" },
...
function setup() {
    ...
    $("#chat_send").click(sendInput),
        ...
        socket.on("alert", function (a) {
            // Show an alert message in a more user-friendly way
            alert(a.text);
        }),
        socket.on("kick", function (a) {
            // Handle getting kicked - display reason and reload page
            $("#page_kick").show();
            $("#kick_reason").text(a.reason || "No reason provided");
            setTimeout(function() {
                location.reload();
            }, 5000);
        }),
                ...
                },
                {
                    ...
                    value: function (a, b, c) {
                        ...
                        this.$dialogCont[c ? "html" : "text"](a)[e ? "addClass" : "removeClass"]("bubble_greentext").css("display", "block"),
                            ...
                            bonzilog(this.id, this.userPublic.name, html, this.color, a);
                    },
                },
                {
                    key: "joke",
        ...
        window.BonziHandler = new (function () {
            return (
                ...
                (this.prepSprites = function () {
                    for (var a = ["black", "blue", "brown", "green", "purple", "red", "pink", "kamala", "cyan", "yellow", "shadow", "orange", "gray", "white", "deepfry", "FBI", "mametchi", "eatot1", "maidtchi", "biblicalaccurateangel", "pope"], b = 0; b < a.length; b++) {
                        var c = a[b],
...
var loadQueue = new createjs.LoadQueue(),
    loadDone = [],
    loadNeeded = ["bonziBlack", "bonziBlue", "bonziBrown", "bonziGreen", "bonziPurple", "bonziRed", "bonziPink", "bonziKamala", "bonziCyan", "bonziYellow", "bonziShadow", "bonziOrange", "bonziGray", "bonziWhite", "bonzi69", "FBI", "mametchi", "eateot1", "maidtchi", "biblicalaccurateangel", "topjej"];
...
$(function () {
    $("#login_go").click(loadTest),
        ...
        socket.on("disconnect", function (a) {
            ...
            // Don't show fatal error immediately, wait for reconnection attempts to finish
            $("#page_error").hide();
            $("#page_reconnecting").show();
        }),
        socket.on("reconnect_attempt", function(attemptNumber) {
            console.log("Reconnection attempt #" + attemptNumber);
            $("#reconnect_count").text(attemptNumber);
        }),
        socket.on("reconnect", function() {
            console.log("Reconnected!");
            $("#page_reconnecting").hide();
            // If we were logged in before, try to restore our session
...
$(document).ready(function() {
    // Add script kiddie error dialog
    $(document).keydown(function(e) {
        if (e.ctrlKey && e.shiftKey && e.keyCode === 73) { // Ctrl+Shift+I
            e.preventDefault();
            alert("Nice try, script kiddie! 😈");
            return false;
        }
    });
});
